...
class Config(object):
    ...
    # 端口: 3306
    # 数据库: test
    SQLALCHEMY_DATABASE_URI = "mysql://root:990421@127.0.0.1:3306/hxDST"
    # SQLALCHEMY_DATABASE_URI = "mysql://root:root@192.168.3.223:3306/hxDST"
    # 设置sqlalchemy自动跟踪数据库
...
def chat():
    ...
    message = get_data.get('content', '')
    in_userInfo = get_data.get('user_info', '')
    in_mode = get_data.get('mode', '')
    in_history_list = get_data.get('history_list', '')
    sid = get_data.get('sessionid', -1)
    print(message,in_userInfo,in_mode,in_history_list)
    if sid == -1:
        return_dict = {
            "return_code": 202,
        ...
        status,text,user_info,intent,intent_id,groupId,states,input_para = chatbot.reply(message)

        # print(text,user_info,intent,states)
        ...
        #input_para
        return_dict['content'] = input_para['content']
        return_dict['user_info'] = input_para['user_info']
        return_dict['mode'] = input_para['mode']
        return_dict['history_list'] = input_para['history_list']

        # return_para = {}
        return_dict['tagid'] = intent_id
        return_dict['tag'] = intent
        return_dict['tag_single'] = intent
        return_dict['prob'] = 1
        return_dict['content'] = message
        return_dict['content_new'] = message
        return_dict['slot'] = None
        return_dict['entity'] = []
        return_dict['status'] = status

        # return_dict['input_para'] = input_para
        # return_dict['return_para'] = return_para
        ret_data = json.dumps(return_dict, ensure_ascii=False)

        round = {'airport':"URC",'mode':"text",'question':message,'parseInfo':{'intent':intent,'groupId':groupId}}
        chatbot.addRound(round)
...
if __name__ == '__main__':
    ...
    # app.logger.handlers = gunicorn_logger.handlers
    # app.logger.setLevel(gunicorn_logger.level)
    # app.run(debug=True,host='10.1.1.6 ',port =30018)
